...
class DashboardProvider {
  Future<List<ComicModel>> getComicListHot() async {
    try {
      ...
      DataSnapshot snapshot =
          (await invoiceRef.child('Truyen').once()).snapshot;
      ...
      comics.sort(
          (a, b) => int.parse(b.luotXem!).compareTo(int.parse(a.luotXem!)));
      print(comics);
      return comics;
    } catch (e) {
  ...
  Future<List<ImageChap>> getImageChapComicById(
      String id, String idChuong) async {
    try {
      DatabaseReference comicRef =
          stringFirebase.databaseconnect.ref(stringFirebase.StringHost);
      DataSnapshot snapshot = (await comicRef
              .child('Truyen')
              .child(id)
              .child('ChuongTruyen')
              .child(idChuong)
  ...
  Future<ComicModel?> getComicById(String id) async {
    try {
      DatabaseReference comicRef =
          stringFirebase.databaseconnect.ref(stringFirebase.StringHost);
      DataSnapshot snapshot =
          (await comicRef.child('Truyen').child(id).once()).snapshot;
  ...
  // String getTinhTrangFromStatus(ComicStatus status) {
  //   switch (status) {
  //     case ComicStatus.update:
  //       return 'Đang cập nhật';
  //     case ComicStatus.success:
  //       return 'Hoàn thành';
  //     default:
  //       return '';
  //   }
